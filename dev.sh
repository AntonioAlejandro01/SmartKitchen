...
function checkJars() {
    FILE=$(sudo ls "./$1/target/" | grep ".*\.jar$")
    if [ "" != "$FILE" ]; then # existe el fichero
       echo -e "Jar File $1 : $GREEN EXIST $NOCOLOR";
    else
        cd "./$1"
...
sudo echo -e "$WHITE Developer mode $NOCOLOR"
...
docker-compose -f docker-compose.development.yml up$detached $build